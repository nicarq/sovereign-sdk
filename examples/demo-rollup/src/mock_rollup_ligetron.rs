...
#[derive(Default)]
pub struct MockDemoRollupLigetron<M> {
    phantom: std::marker::PhantomData<M>,
}

/// The default spec of the rollup
pub type MockRollupSpecLigetron<M> =
    ConfigurableSpec<MockDaSpec, Ligetron, MockZkvm, MultiAddressEvm, M>;

impl RollupBlueprint<Native> for MockDemoRollupLigetron<Native>
where
    MockRollupSpecLigetron<Native>: PluggableSpec,
    <MockRollupSpecLigetron<Native> as Spec>::Address: FromVmAddress<EthereumAddress>,
{
    type Spec = MockRollupSpecLigetron<Native>;
    type Runtime = Runtime<Self::Spec>;
}

impl RollupBlueprint<WitnessGeneration> for MockDemoRollupLigetron<WitnessGeneration>
where
    MockRollupSpecLigetron<WitnessGeneration>: PluggableSpec,
    <MockRollupSpecLigetron<WitnessGeneration> as Spec>::Address: FromVmAddress<EthereumAddress>,
{
    type Spec = MockRollupSpecLigetron<WitnessGeneration>;
    type Runtime = Runtime<Self::Spec>;
}

#[async_trait]
impl FullNodeBlueprint<Native> for MockDemoRollupLigetron<Native> {
    type DaService = StorableMockDaService;

    type StorageManager =
        NativeStorageManager<MockDaSpec, <MockRollupSpecLigetron<Native> as Spec>::Storage>;
