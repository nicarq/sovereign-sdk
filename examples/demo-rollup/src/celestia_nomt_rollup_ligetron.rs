...
use sov_ligetron_adapter::{Ligetron, LigetronCryptoSpec};
use sov_ligetron_adapter::host::LigetronHost;
...
/// Rollup with CelestiaDa (Ligetron inner VM) and NOMT storage
#[derive(Default)]
pub struct CelestiaNomtDemoRollupLigetron<M> {
    phantom: std::marker::PhantomData<M>,
}

type Hasher = <LigetronCryptoSpec as CryptoSpec>::Hasher;
type NativeStorage =
    NomtProverStorage<DefaultStorageSpec<Hasher>, <CelestiaSpec as DaSpec>::SlotHash>;

type CelestiaRollupSpecLigetron<M> = ConfigurableSpec<
    CelestiaSpec,
    Ligetron,
    MockZkvm,
    MultiAddressEvm,
...
impl RollupBlueprint<Native> for CelestiaNomtDemoRollupLigetron<Native>
where
    CelestiaRollupSpecLigetron<Native>: PluggableSpec,
    <CelestiaRollupSpecLigetron<Native> as Spec>::Address: FromVmAddress<EthereumAddress>,
{
    type Spec = CelestiaRollupSpecLigetron<Native>;
    type Runtime = Runtime<Self::Spec>;
}

impl RollupBlueprint<WitnessGeneration> for CelestiaNomtDemoRollupLigetron<WitnessGeneration>
where
    CelestiaRollupSpecLigetron<WitnessGeneration>: PluggableSpec,
    <CelestiaRollupSpecLigetron<WitnessGeneration> as Spec>::Address: FromVmAddress<EthereumAddress>,
{
    type Spec = CelestiaRollupSpecLigetron<Native>;
    type Runtime = Runtime<Self::Spec>;
}
...
#[async_trait]
impl FullNodeBlueprint<Native> for CelestiaNomtDemoRollupLigetron<Native> {
    ...
    async fn create_prover_service(
        &self,
        prover_config: RollupProverConfig<Ligetron>,
        rollup_config: &RollupConfig<<Self::Spec as Spec>::Address, Self::DaService>,
        _da_service: &Self::DaService,
    ) -> Self::ProverService {
        let (wasm, prover_config_disc) = prover_config.split();
        let inner_vm = LigetronHost::from_args(&wasm);
...
impl WalletBlueprint<Native> for CelestiaNomtDemoRollupLigetron<Native> {}
