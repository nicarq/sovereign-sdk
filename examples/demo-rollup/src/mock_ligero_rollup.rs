...
#[derive(Default)]
pub struct MockLigeroDemoRollup<M> {
    phantom: std::marker::PhantomData<M>,
}

/// The default spec of the rollup (Ligero/Ligetron)
pub type MockLigeroRollupSpec<M> =
    ConfigurableSpec<MockDaSpec, Ligetron, MockZkvm, MultiAddressEvm, M>;

impl RollupBlueprint<Native> for MockLigeroDemoRollup<Native>
where
    MockLigeroRollupSpec<Native>: PluggableSpec,
    <MockLigeroRollupSpec<Native> as Spec>::Address: FromVmAddress<EthereumAddress>,
{
    type Spec = MockLigeroRollupSpec<Native>;
    type Runtime = Runtime<Self::Spec>;
}

impl RollupBlueprint<WitnessGeneration> for MockLigeroDemoRollup<WitnessGeneration>
where
    MockLigeroRollupSpec<WitnessGeneration>: PluggableSpec,
    <MockLigeroRollupSpec<WitnessGeneration> as Spec>::Address: FromVmAddress<EthereumAddress>,
{
    type Spec = MockLigeroRollupSpec<WitnessGeneration>;
    type Runtime = Runtime<Self::Spec>;
}

#[async_trait]
impl FullNodeBlueprint<Native> for MockLigeroDemoRollup<Native> {
    type DaService = StorableMockDaService;

    type StorageManager =
        NativeStorageManager<MockDaSpec, <MockLigeroRollupSpec<Native> as Spec>::Storage>;
