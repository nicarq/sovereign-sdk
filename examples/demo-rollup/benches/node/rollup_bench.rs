...
use sov_modules_api::{
    BatchSequencerOutcome, BatchSequencerReceipt, CryptoSpec, EncodeCall, FullyBakedTx, Gas,
    GasSpec, GasUnit, RawTx, Spec, ZkVerifier, Zkvm,
};
...
use sov_test_utils::{
    TestProver, TestSequencer, TestStorageSpec, TestUser, TEST_DEFAULT_MAX_FEE,
...
type BenchSpec = sov_modules_api::configurable_spec::ConfigurableSpec<
...
type TestPrivateKey = <<BenchSpec as Spec>::CryptoSpec as CryptoSpec>::PrivateKey;
...
fn bake_bank_tx(
    ...
    pk: &<<BenchSpec as Spec>::CryptoSpec as CryptoSpec>::PrivateKey,
    nonce: u64,
) -> FullyBakedTx {
...
fn prefill_state(
    ...
    let create_token_msg = bake_bank_tx(
        sov_bank::CallMessage::<BenchSpec>::CreateToken {
            ...
            admins: vec![rollup_mega_admin.address()]
                .try_into()
...
fn stf_apply_slot_bench(c: &mut Criterion) {
    ...
    let user_stake = <BenchSpec as Spec>::Gas::from(TEST_DEFAULT_USER_STAKE);
    let user_stake_value = user_stake.value(&BenchSpec::initial_base_fee_per_gas());
    ...
    let minimal_config = MinimalZkGenesisConfig::<BenchSpec>::from_args(
        ...
        <<<BenchSpec as Spec>::InnerZkvm as Zkvm>::Verifier as ZkVerifier>::CodeCommitment::default(
        ),
        <<<BenchSpec as Spec>::OuterZkvm as Zkvm>::Verifier as ZkVerifier>::CodeCommitment::default(
        ),
    );
