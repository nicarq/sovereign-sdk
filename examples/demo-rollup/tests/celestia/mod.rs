...
async fn submit_blobs_increasing_size() -> anyhow::Result<()> {
    ...
    let token_deployer: PrivateKeyAndAddress<
        DefaultSpec<CelestiaSpec, Risc0Verifier, Risc0Verifier, Native>,
    ...
    let messages = generate_call_message::<
        DefaultSpec<CelestiaSpec, Risc0Verifier, Risc0Verifier, Native>,
    ...
    for (idx, message) in messages.into_iter().enumerate() {
        ...
        let tx = Transaction::<DefaultSpec<CelestiaSpec, Risc0Verifier, Risc0Verifier, Native>>::new_signed_tx(
