...
pub(crate) mod files {
    ...
    use super::*;
    use crate::da_service::{raw_blob_from_data, CelestiaFee};
    ...
    pub(crate) async fn submit_blobs(
        ...
        let blob_sizes = blobs.iter().map(|blob| blob.data.len()).sum::<usize>();

        let fee = CelestiaFee::estimated(blob_sizes + 1000);
        let mut tx_config = celestia_rpc::TxConfig::default();
        tx_config
            .with_gas_price(fee.fee_rate() as f64 * 2.0)
            .with_gas(fee.gas_estimate() * 2);
