...
impl<'a> ::arbitrary::Arbitrary<'a> for CelestiaAddress {
    fn arbitrary(u: &mut arbitrary::Unstructured<'a>) -> arbitrary::Result<Self> {
        const SIZE: usize = 20 + std::mem::size_of::<u32>();
        let bytes = <[u8; SIZE]>::arbitrary(u)?;
        Ok(borsh::from_slice(&bytes).expect("Failed to deserialize generated bytes as a Celestia Address. This is a bug in the arbitrary implementation."))
    }
}
