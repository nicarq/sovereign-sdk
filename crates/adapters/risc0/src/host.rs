...
impl<'a> Risc0Host<'a> {
    ...
    pub fn run(&mut self) -> anyhow::Result<Receipt> {
        ...
        // (e.g., IPC via r0vm) selected by environment variables such as `RISC0_PROVER`.
        // This path is enabled when the crate is built with the `native` feature (which
        // also enables the `prove` feature on `risc0-zkvm`).
        #[cfg(feature = "native")]
        {
            let mut env = add_benchmarking_callbacks(ExecutorEnvBuilder::default());
            #[cfg(feature = "bincode")]
        ...
        // Fallback for builds without `native` feature: use in-process proving.
