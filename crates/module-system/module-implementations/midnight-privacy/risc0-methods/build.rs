...
fn main() {
    ...
    if should_skip_guest_build("risc0") {
        println!("cargo:warning=Skipping risc0 guest build for midnight-privacy");
        let out_dir = std::env::var_os("OUT_DIR").unwrap();
        let out_dir = std::path::Path::new(&out_dir);
        let methods_path = out_dir.join("methods.rs");

        let contents = r#"
            pub const SPEND_PATH: &str = "";
            pub const SPEND_ELF: &[u8] = b"";
        "#;
        ...
        let guest_options = risc0_build::GuestOptionsBuilder::default()
            ...
            .build()
            .unwrap();
        guest_pkg_to_options.insert("midnight-privacy-risc0-guest-spend", guest_options);
