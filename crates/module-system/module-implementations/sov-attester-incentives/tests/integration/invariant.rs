...
fn setup_invariant_tests() -> (TestRunner<RT, S>, TestAttester<S>, u64) {
    ...
    // Run empty slots.
    runner.execute_slots::<TestAttesterIncentives>(SlotTestCase::empty_slots(
        TEST_ROLLUP_FINALITY_PERIOD as usize,
    ));
                        ...
                        state.inner().events().iter().any(|event| {
                            event.downcast_ref::<Event<S>>()
                                == Some(&Event::ProcessedValidAttestation {
                                    attester: genesis_attester_address,
                                })
                        },),
                        "No processed valid attestation event emitted. Slot height {i}"
                    );
...
fn test_cannot_attest_below_max_attested_height() {
    ...
    runner.execute_slots(
        vec![SlotTestCase::from_rewarded_batch(
            ...
            Box::new(move |state| {
                // Ensure that the `MAX_ATTESTED_HEIGHT - TEST_ROLLUP_FINALITY_PERIOD > 1` - this triggers the revert
                let max_attested_height = TestAttesterIncentives::default()
                    .maximum_attested_height
                    .get(state)
                    .unwrap_infallible()
