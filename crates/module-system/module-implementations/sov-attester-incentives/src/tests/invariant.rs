...
#[test]
fn test_transition_invariant() -> Result<(), Infallible> {
    ...
    let (exec_vars, mut state) = ExecutionSimulationVars::execute(
        20,
        &module,
        &mut storage_manager,
        &sequencer,
    ...
    // Process a valid attestation for the first transition *should fail*
    {
        ...
        let attestation = Attestation {
            ...
            slot_hash: [(init_height_usize + 1).try_into().unwrap(); 32].into(),
    ...
    for i in 0..usize::try_from(DEFAULT_ROLLUP_FINALITY + 1).unwrap() {
        ...
        let new_attestation = Attestation {
            ...
            slot_hash: [(new_height + i).try_into().unwrap(); 32].into(),
    ...
    let old_attestation = Attestation {
        ...
        slot_hash: [(new_height + finality_usize + 1).try_into().unwrap(); 32].into(),
