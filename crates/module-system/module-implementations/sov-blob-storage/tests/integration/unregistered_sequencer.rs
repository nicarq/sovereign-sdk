...
>(
    ...
    (0..num_blobs)
        .map(|_| {
            ...
            let raw_tx = runner.query_state(|state| tx.to_raw_tx::<RT>(nonces, state));
...
fn blobs_from_non_registered_sequencers_are_limited_to_set_amount() {
    ...
    // Send them
    let result = runner.execute::<RelevantBlobs<MockBlob>, SequencerRegistry<S, MockDaSpec>>(
        unregistered_blobs,
        None,
    );
...
fn blobs_from_non_registered_sequencers_are_limited_to_set_amount_soft_confirmation() {
    ...
    // Send them
    let result = runner
        .execute::<RelevantBlobs<MockBlob>, SequencerRegistry<S, MockDaSpec>>(slot_to_send, None);
