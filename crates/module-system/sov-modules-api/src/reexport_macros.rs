...
pub mod macros {
    ...
    ///
    /// The schema generated by the trait allows the borsh-encoding of the type to be dispalyed in a human-readable format,
    /// with the exact formatting controlled by attributes on the fields of the type.
    ///
    ...
    /// assert_eq!(Schema::of::<Unreadable>().display(&serialized), r#"{ name: "foo.txt"}"#);
    /// ```
    ///
    ...
    ///
    /// This is useful when you want to display a foreign type that doesn't implement [`SchemaGenerator`](crate::sov_wallet_format::traits::SchemaGenerator),
    ...
    /// assert_eq!(Schema::of::<Tagged>().display(&serialized), r#"{ data: "foo", tag: "world"}"#);
    /// ```
    ///
    ...
    /// assert_eq!(Schema::of::<File>().display(&serialized), "{ contents: 0x010203}");
    /// ```
    ///
    ...
    /// assert_eq!(Schema::of::<CelestiaAddress>().display(&serialized), "celestia1qyqszqgpqyqszqgpqyqszqgpqyqszqgpqyqszqgpqyqszqgpqyqsagv2r7");
    /// ```
