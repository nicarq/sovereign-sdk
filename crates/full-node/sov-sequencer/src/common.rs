...
pub trait Sequencer: Send + Sync + 'static {
    ...
    /// Queries whether this sequencer is a master (able to accept transactions) or a replica (only
    /// able to respond to read-only API queries)
    async fn is_master(&self) -> bool;

    /// Returns a UUID generated by this instance on startup. Every run of the sequencer will have
    /// a new ID.
