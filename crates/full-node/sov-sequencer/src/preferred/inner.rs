    ...
    Rt: Runtime<S>,
{
    ...
    #[tracing::instrument(skip_all, level = "trace")]
    pub(crate) async fn trigger_batch_production_if_convenient(
        &mut self,
        stop_at_rollup_height: Option<RollupHeight>,
    ) {
    ...
    Rt: Runtime<S>,
{
    pub(crate) async fn process_accept_tx(
        ...
        stop_at_rollup_height: Option<RollupHeight>,
        is_master: bool,
    ) -> Result<oneshot::Receiver<Option<AcceptedTx<Confirmation<S, Rt>>>>, AcceptTxError<S>> {
        ...
        if let Err(batch_creation_error) = self
            .try_to_create_and_start_batch_if_none_in_progress(stop_at_rollup_height, false)
            .await
        {
    ...
    pub(crate) async fn process_do_new_tx(
        ...
        let (
            ...
            tx_changes,
        ) = self.executor.replay_tx(tx_hash, &baked_tx).await;
