...
#[tokio::test(flavor = "multi_thread")]
async fn returns_syncing_at_startup() {
    const FINALIZATION_BLOCKS: u32 = 0;
    let genesis_config =
        HighLevelOptimisticGenesisConfig::generate().add_accounts_with_default_balance(1);
    let admin = genesis_config.additional_accounts[0].clone();
    ...
    // Produce a valid transaction that sets a value. and submit it. This should fail because the node is not synced yet.
    let tx = tx_set_value(&admin.private_key, 0, 1);
    let initial_output = test_rollup
        .api_client
        .accept_tx(&api_types::AcceptTxBody {
            body: BASE64_STANDARD.encode(&tx),
        })
    ...
    let Err(error) = initial_output else {
    ...
    // Finalize some blocks and wait for the node to process them. Then, retry submission and assert that it succeeds
    da_layer.write().await.produce_block().await.unwrap();
    slot_subscription.next().await.unwrap().unwrap();
    da_layer.write().await.produce_block().await.unwrap();
    slot_subscription.next().await.unwrap().unwrap();
    ...
    test_rollup
        ...
        .await
        .expect("Sequencer should be ready to accept txs after finalized blocks are available");
}
