    ...
    fn try_into(self) -> anyhow::Result<aggregated_proof::AggregatedProof> {
        ...
        Ok(aggregated_proof::AggregatedProof::new(
            ...
            aggregated_proof::AggregatedProofPublicData {
                validity_conditions: self
                    .public_data
                    .validity_conditions
                    .iter()
                    .map(|v| decode_b64(v))
                    .collect::<anyhow::Result<Vec<_>>>()?,
                initial_slot_number: self.public_data.initial_slot_number,
                final_slot_number: self.public_data.final_slot_number,
                genesis_state_root: decode_b64(&self.public_data.genesis_state_root)?,
                initial_state_root: decode_b64(&self.public_data.initial_state_root)?,
                final_state_root: decode_b64(&self.public_data.final_state_root)?,
                initial_slot_hash: decode_b64(&self.public_data.initial_slot_hash)?,
                final_slot_hash: decode_b64(&self.public_data.final_slot_hash)?,
