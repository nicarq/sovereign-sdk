...
fn main() -> Result<(), anyhow::Error> {
    ...
    if !risc0_cmd_output.status.success() {
        anyhow::bail!("Failed to get risc0 rustc version. Is risc0 installed? If not you can install it with the `cargo-risczero` tool. Try running `cargo risczero install`");
    }
    ...
    if risc0_rustc_version != native_rustc_version {
        anyhow::bail!(
			"Risc0 rustc version {} does not match native rustc version {}. Please update your Risc0 toolchain or use a rust-toolchain.toml file to force your native compiler to the correct version.",
			risc0_rustc_version,
